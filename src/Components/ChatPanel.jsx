...
function ChatPanel() {
    // Get list of all users from firebase database using getDocs
    // As getting users from db can take some time - meanwhile show 'loading..' for chatList
    ...
    return (
        <div className='bg-white w-[30vw] min-w-[330px]'>
            {/* top bar */}
            <div className='flex justify-between items-center py-2 px-4 bg-background'>
            ...
            {/* chat list */}
            {
                ...
                <div className='bg-white py-3 px-3'>
                    <div className='flex items-center gap-4 py-2 px-3 rounded-lg bg-background'>
                        ...
                        <input type='text' className='bg-background focus-within:outline-none w-full' value={searchQuery} placeholder='Search' onChange={(e) => {setSearchQuery(e.target.value)}}></input>
                    </div>
                    <div className='flex flex-col divide-y py-4 max-h-[calc(100vh-152px)] overflow-y-scroll'>
                        {filteredUsers.map((userObject) => {
                        return <Link key={userObject.id} className={`flex items-center gap-4 p-2 hover:bg-background rounded ${ params?.id === userObject.id && "bg-background"}`} to={`/${userObject.id}`}>
